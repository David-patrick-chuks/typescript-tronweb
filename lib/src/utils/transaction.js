...
const { TransferContract, FreezeBalanceContract, UnfreezeBalanceContract, WithdrawBalanceContract, FreezeBalanceV2Contract, UnfreezeBalanceV2Contract, WithdrawExpireUnfreezeContract, DelegateResourceContract, UnDelegateResourceContract, } = require('../proto/core/contract/balance_contract');
const { TransferAssetContract, ParticipateAssetIssueContract, AssetIssueContract, UpdateAssetContract } = require('../proto/core/contract/asset_issue_contract');
const { TriggerSmartContract, ClearABIContract, UpdateEnergyLimitContract, UpdateSettingContract, CreateSmartContract, SmartContract, } = require('../proto/core/contract/smart_contract');
const { ResourceCode } = require('../proto/core/contract/common');
const { WitnessCreateContract, VoteWitnessContract, } = require('../proto/core/contract/witness_contract');
const { UpdateBrokerageContract, } = require('../proto/core/contract/storage_contract');
const { AccountCreateContract, AccountUpdateContract, SetAccountIdContract, AccountPermissionUpdateContract, } = require('../proto/core/contract/account_contract');
const { ProposalCreateContract, ProposalDeleteContract, ProposalApproveContract, } = require('../proto/core/contract/proposal_contract');
const { ExchangeCreateContract, ExchangeInjectContract, ExchangeWithdrawContract, ExchangeTransactionContract, } = require('../proto/core/contract/exchange_contract');
...
const fromHexString = (hexString) => {
    ...
    return new Uint8Array(
    // @ts-ignore
    index_1.default.address.toHex(hexString).match(/.{1,2}/g).map((byte) => parseInt(byte, 16)));
};
...
const buildCreateSmartContract = (value, options) => {
    let params = (value === null || value === void 0 ? void 0 : value.new_contract) ? Object.assign({
        owner_address: value.owner_address,
        call_token_value: value.call_token_value,
        token_id: value.token_id,
    }, value.new_contract) : value;
    let { owner_address, consume_user_resource_percent, origin_energy_limit, abi, bytecode = '', name: contracName, parameter = '', call_value, call_token_value, token_id, origin_address, } = params;
    ...
    if (abi) {
        ...
        if (typeof abi === 'string') {
            ...
            abiJson = (abi === null || abi === void 0 ? void 0 : abi.entrys) || []; // abi could be an empty object if origin abi is `[]`;
        }
...
const txJsonToPbWithArgs = (transaction, args = {}, options = {}) => {
    const rawData = transaction['raw_data'];
    const contractJson = rawData.contract[0];
    const transactionObj = contractJsonToProtobuf(contractJson, args, { Permission_id: args === null || args === void 0 ? void 0 : args.Permission_id });
    const rawDataObj = transactionObj.getRawData();
    rawDataObj.setRefBlockBytes(fromHexString(rawData.ref_block_bytes));
    rawDataObj.setRefBlockHash(fromHexString(rawData.ref_block_hash));
    // for memo
...
const compareTransaction = (transaction, transactionPb) => {
    ...
    const rawDataHex = (0, bytes_1.byteArray2hexStr)(rawDataBytes);
    const txID = (0, ethersUtils_1.sha256)(rawDataBytes);
...
const txPbToRawDataHex = (pb) => {
    return (0, bytes_1.byteArray2hexStr)(pb.getRawData().serializeBinary());
};
