...
class Trx extends _base_1.WithTronwebAndInjectpromise {
    ...
    static ecRecover(transaction) {
        var _a;
        if (!(0, transaction_1.txCheck)(transaction)) {
            throw new Error('Invalid transaction');
        }
        if (!((_a = transaction.signature) === null || _a === void 0 ? void 0 : _a.length)) {
            throw new Error('Transaction is not signed');
        }
        if (transaction.signature.length === 1) {
            const tronAddress = (0, crypto_1.ecRecover)(transaction.txID, transaction.signature[0]);
            return __1.default.address.fromHex(tronAddress);
        }
        return transaction.signature.map((sig) => {
            const tronAddress = (0, crypto_1.ecRecover)(transaction.txID, sig);
            return __1.default.address.fromHex(tronAddress);
        });
    }
