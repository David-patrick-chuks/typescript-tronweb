...
describe('TronWeb.transactionBuilder', function () {
    ...
    before(async function () {
        ...
        isAllowSameTokenNameApproved = await isProposalApproved(
            tronWeb,
    ...
    describe('#sendTrx()', function () {
        it('should send 10 trx from default address to accounts[1]', async function () {
            const params: [
                ...
                [accounts.b58[1], 10, undefined, {permissionId: 2}],
                [accounts.b58[1], 10, undefined],
            ];
            for (const param of params) {
                ...
                assert.equal(
                    ...
                    param[3] ? param[3]['permissionId'] : 0,
                );
            }
        });

        it('should send 10 trx from accounts[0] to accounts[1]', async function () {
            const params: [string, number, string, {permissionId?: number}?][] =
                [
    ...
    describe('#createToken()', function () {
        ...
        it('should allow accounts[8] to create a TestToken with voteScore and precision', async function () {
            if (isAllowSameTokenNameApproved) {
                ...
                for (let i = 0; i < 2; i++) {
                    ...
                    const tokenList =
                        await tronWeb.trx.getTokensIssuedByAddress(
                            accounts.b58[8 + i],
                        );
        ...
        it('should create a TestToken without freezing anything in 3.6.0', async function () {
            if (
                tronWeb.fullnodeSatisfies('^3.6.0') &&
                !/127\.0\.0\.1/.test(FULL_NODE_API)
            ) {
                const options = getTokenOptions();
                options.totalSupply = '100';
        ...
        describe('#createAsset()', function () {
            ...
            it('should allow accounts[2] to create a TestToken', async function () {
                const options = getTokenOptions();
                const transaction =
                    await tronWeb.transactionBuilder.createAsset(
                        options,
                        accounts.b58[2],
                    );
                const parameter = txPars(transaction);
    ...
    describe('#updateAccount()', function () {
        it('should update accounts[3]', async function () {
            ...
            const params: [string, string, {permissionId?: number}?][] = [
            ...
            for (const param of params) {
                const transaction =
                    await tronWeb.transactionBuilder.updateAccount(...param);
                const parameter = txPars(transaction);
                ...
                assert.equal(
                    transaction.raw_data.contract[0].Permission_id || 0,
                    param[2] ? param[2]['permissionId'] : 0,
                );
            }
        });
    ...
    describe('#setAccountId()', function () {
        it('should set account id accounts[4]', async function () {
            ...
            for (const id of ids) {
                const accountId = TronWeb.toHex(id);
                const transaction =
                ...
                assert.equal(
                    parameter.type_url,
                    'type.googleapis.com/protocol.SetAccountIdContract',
                );
            }
        });
        ...
        it('should throw invalid account id error', async function () {
            ...
            for (const id of ids)
                await assertThrow(
                    tronWeb.transactionBuilder.setAccountId(
                        id,
                        accounts.b58[4],
                    ),
    ...
    describe('#updateToken()', function () {
        let tokenOptions: ReturnType<typeof getTokenOptions>;
        ...
        before(async function () {
            ...
            let tokenList;
            while (!tokenList)
                tokenList = await tronWeb.trx.getTokensIssuedByAddress(
                    accounts.b58[2],
                );

            // if (isAllowSameTokenNameApproved)
            //     tokenID = tokenList[tokenOptions.name].id;
            // else tokenID = tokenList[tokenOptions.name].name;
        });
        ...
        it('should allow accounts[2] to update a TestToken', async function () {
            for (let i = 0; i < 2; i++) {
                ...
                const transaction =
                    await tronWeb.transactionBuilder.updateToken(
                        UPDATED_TEST_TOKEN_OPTIONS,
                        accounts.b58[2],
                    );
                const parameter = txPars(transaction);
                ...
                await assertEqualHex(
                    ...
                    UPDATED_TEST_TOKEN_OPTIONS.description,
                );
                await assertEqualHex(
                    parameter.value.url,
                    UPDATED_TEST_TOKEN_OPTIONS.url,
                );
                assert.equal(parameter.value.owner_address, accounts.hex[2]);
                assert.equal(
                    parameter.type_url,
                    'type.googleapis.com/protocol.UpdateAssetContract',
                );
                assert.equal(
                    transaction.raw_data.contract[0].Permission_id || 0,
                    UPDATED_TEST_TOKEN_OPTIONS.permissionId || 0,
                );
            }
        });
        ...
        describe('#updateAsset()', async function () {
            it('should allow accounts[2] to update a TestToken', async function () {
                const transaction =
                    await tronWeb.transactionBuilder.updateAsset(
                        UPDATED_TEST_TOKEN_OPTIONS,
                        accounts.b58[2],
                    );
                const parameter = txPars(transaction);
    ...
    describe('#purchaseToken()', function () {
        ...
        before(async function () {
            ...
            assert.equal(
                tokenList[tokenOptions.name].abbr,
                tokenOptions.abbreviation,
            );
        });
        ...
        it('should allow accounts[2] to purchase a token created by accounts[5]', async function () {
            ...
            for (const param of params) {
                ...
                const transaction =
                    await tronWeb.transactionBuilder.purchaseToken(...param);
                const parameter = txPars(transaction);
                assert.equal(transaction.txID.length, 64);
                ...
                assert.equal(
                    transaction.raw_data.contract[0].Permission_id || 0,
                    param[4] ? param[4]['permissionId'] : 0,
                );
            }
        });
    ...
    describe('#createProposal', async function () {
        ...
        before('Become a witness', async () => {
            await broadcaster(
                tronWeb.transactionBuilder.applyForSR(
                    ADDRESS_HEX,
                    'https://example.com/',
                ),
                PRIVATE_KEY,
        ...
        it('should allow the SR account to create a new proposal as a single object', async function () {
            ...
            for (const input of inputs) {
                const transaction =
                    await tronWeb.transactionBuilder.createProposal(...input);

                const parameter = txPars(transaction);

                assert.equal(parameter.value.owner_address, ADDRESS_HEX);
                assert.equal(
                    parameter.value.parameters[0].value,
                    parameters[0].value,
                );
                assert.equal(
                    parameter.type_url,
        ...
        it('should throw if issuer address is invalid', async function () {
            await assertThrow(
                tronWeb.transactionBuilder.createProposal(
                    parameters,
                    'sadasdsffdgdf',
                ),
                'Invalid issuer address provided',
        ...
        it('should throw if the issuer address is not an SR', async function () {
            await assertThrow(
                ...
                null,
                `Witness[${accounts.hex[1]}] not exists`,
            );
        });
    ...
    describe('#deleteProposal', async function () {
        ...
        beforeEach(async function () {
            ...
            await broadcaster(
                tronWeb.transactionBuilder.createProposal(parameters),
                PRIVATE_KEY,
            );
        ...
        after(async function () {
            ...
            for (const proposal of proposals)
                if (proposal.state !== 'CANCELED')
                    await broadcaster(
                        tronWeb.transactionBuilder.deleteProposal(
        ...
        it('should allow the SR to delete its own proposal', async function () {
            const params: [number, undefined, {permissionId?: number}?][] = [
                [proposals[0].proposal_id, undefined, {permissionId: 2}],
                [proposals[0].proposal_id, undefined],
            ];
            for (const param of params) {
                const transaction =
                    await tronWeb.transactionBuilder.deleteProposal(...param);
                const parameter = txPars(transaction);

                assert.equal(parameter.value.owner_address, ADDRESS_HEX);
                assert.equal(
                    parameter.value.proposal_id,
    ...
    describe.skip('#applyForSR', async function () {
        ...
        it('should allow accounts[0] to apply for SR', async function () {
            const transaction = await tronWeb.transactionBuilder.applyForSR(
                accounts.b58[20],
                url,
            );
            const parameter = txPars(transaction);
    ...
    describe('#freezeBalance', async function () {
        it('should allows accounts[1] to freeze its balance', async function () {
            const params: [
                ...
                [100e6, 3, 'BANDWIDTH', accounts.b58[1], undefined],
            ];

            for (const param of params) {
                const transaction =
                    await tronWeb.transactionBuilder.freezeBalance(...param);
                ...
                assert.equal(
                    parameter.type_url,
                    'type.googleapis.com/protocol.FreezeBalanceContract',
                );
                assert.equal(
                    transaction.raw_data.contract[0].Permission_id || 0,
                    param[5] ? param[5]['permissionId'] : 0,
                );
            }
        });
    ...
    describe.skip('#vote', async function () {
        ...
        before(async function () {
            await broadcaster(
                tronWeb.transactionBuilder.applyForSR(accounts.b58[0], url),
                accounts.pks[0],
            );
            await broadcaster(
        ...
        it('should allows accounts[1] to vote for accounts[0] as SR', async function () {
            ...
            assert.equal(parameter.value.owner_address, accounts.hex[1]);
            assert.equal(
                parameter.value.votes[0].vote_address,
                accounts.hex[0],
            );
    ...
    describe('#createSmartContract', async function () {
        it('should create a smart contract with default parameters', async function () {
            ...
            for (let i = 0; i < 2; i++) {
                ...
                assert.equal(
                    tx.raw_data.contract[0].parameter.value.new_contract
                ...
                assert.equal(
                    tx.raw_data.contract[0].Permission_id || 0,
                    options.permissionId || 0,
                );
            }
        });
    ...
    describe('#triggerConstantContract', async function () {
        ...
        it('should trigger constant contract successfully', async function () {
            ...
            for (let i = 0; i < 2; i++) {
                ...
                assert.equal(
                    transaction.transaction.raw_data.contract[0]
                        .Permission_id || 0,
                    options.permissionId || 0,
                );
            }
    ...
    describe('#clearabi', async function () {
        ...
        it('should clear contract abi', async function () {
            ...
            // clear abi
            transaction = await tronWeb.transactionBuilder.clearABI(
                contractAddress,
                ownerAddress,
            );
            assert.isTrue(
                !transaction.visible &&
                    transaction.raw_data.contract[0].parameter.type_url ===
                        'type.googleapis.com/protocol.ClearABIContract',
            );
    ...
    describe('#updateBrokerage', async function () {
        ...
        it('should throw invalid brokerage provided error', async function () {
            await assertThrow(
                tronWeb.transactionBuilder.updateBrokerage(
                    // Intentionally invalid
                    // @ts-ignore
                    null,
                    accounts.hex[1],
    ...
    describe('#createTokenExchange', async function () {
        ...
        it('should create token exchange', async function () {
            ...
            assert.equal(transaction.txID.length, 64);
            assert.equal(
                TronWeb.toUtf8(parameter.value.first_token_id),
                tokenNames[0],
            );
            assert.equal(
                TronWeb.toUtf8(parameter.value.second_token_id),
                tokenNames[1],
            );
            assert.equal(
                parameter.type_url,
    ...
    describe('Alter existent transactions', async function () {
        ...
        describe('#addUpdateData', async function () {
            it('should add a data field', async function () {
                ...
                assert.equal(
                    tronWeb.toUtf8(unconfirmedTx.raw_data.data!),
                    data,
                );
            });
        });
        ...
        describe('#alterTransaction', async function () {
            ...
            it('should alter the transaction adding a data field', async function () {
                ...
                const previousId = transaction.txID;
                const data = 'Sending money to Bill.';
                transaction = await tronWeb.transactionBuilder.alterTransaction(
                    transaction,
                    {data},
                );
                const id = transaction.txID;
