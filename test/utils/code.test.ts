...
describe('TronWeb.utils.code', function () {
        ...
        it('should convert a string to an array of bytes', async function () {
            ...
            assert.isTrue(
                tronWeb.utils.code.arrayEquals(
                    tronWeb.utils.code.stringToBytes(a),
    ...
    describe('#isHexChar()', function () {
        it('should verify that an hex char is a valid hex char', async function () {
            ...
            assert.isTrue(tronWeb.utils.code.isHexChar('0'));
            assert.isTrue(tronWeb.utils.code.isHexChar('e'));
    ...
    describe('#hexStr2byteArray()', function () {
        it('should verify that an hex char is a valid hex char', async function () {
            ...
            assert.isTrue(
                tronWeb.utils.code.arrayEquals(
                    tronWeb.utils.code.hexStr2byteArray(
                        '49206C6F7665206461726B20636F6D6564696573',
                    ),
                    [
                        73, 32, 108, 111, 118, 101, 32, 100, 97, 114, 107, 32,
                        99, 111, 109, 101, 100, 105, 101, 115,
                    ],
                ),
            );
        });
    ...
    describe('#strToDate()', function () {
        it('should verify that an hex char is a valid hex char', async function () {
            ...
            assert.isTrue(
                regex.test(tronWeb.utils.code.strToDate(input).toString()),
            );
    ...
    describe('#getStringType()', function () {
        it('should return the type of a string', async function () {
            ...
            assert.equal(
                tronWeb.utils.code.getStringType(
